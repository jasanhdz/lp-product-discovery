...
export default function SignInPage() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // 2) Fetch the user profile from our profiles table
      const { data: profile } = await supabase.from('profiles').select('*').eq('id', userId).single()

      const displayName = profile?.alias || profile?.first_name || email.split('@')[0]

      // 3) Dispatch to Redux with the full profile
      dispatch(
        loginSuccess({
          user: {
            id: userId,
  ...
  const handleGoogleLogin = async () => {
    // Flag so App.jsx knows this was a LOGIN attempt (not registration)
